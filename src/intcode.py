...
class Intcode():
    ...
    def runProgram(self, program: list, inputStream: list = []):
        ...
        while self.eop is False:
            ...
            if opcode in self.opcodes:
                self.opcodes[opcode](instruction[:-2])

        return self.retVal

    def getOperand(self, value: int, mode: ParameterMode):
        if mode == ParameterMode.Position:
            return self.program[value]
        elif mode == ParameterMode.Immediate:
            return value

    def opcode1(self, modes: str):
        src0 = self.getOperand(self.program[self.ip + 1], ParameterMode(int(modes[-1])))
        src1 = self.getOperand(self.program[self.ip + 2], ParameterMode(int(modes[-2])))
    ...
    def opcode5(self, modes: str):
        src0    = self.getOperand(self.program[self.ip + 1], ParameterMode(int(modes[-1])))
        jumpLoc = self.getOperand(self.program[self.ip + 2], ParameterMode(int(modes[-2])))
        if self.verbose:
